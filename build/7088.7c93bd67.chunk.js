(self["webpackChunkmy_project"] = self["webpackChunkmy_project"] || []).push([[7088],{
...
const AuthResponse = () => {
  const {
    params: { authResponse }
  } = (0,react_router/* useRouteMatch */.W5)("/auth/login/:authResponse");
  ...
  const fetchUserInfo = (0,react.useCallback)(async () => {
    try {
      ...
      if (jwtToken) {
        helper_plugin_esm/* auth */.j2.setToken(jwtToken, true);
        const requestUrl = "/admin/users/me";
        const {
          data: { data }
        } = await get(requestUrl);
        helper_plugin_esm/* auth */.j2.setUserInfo(data, true);
